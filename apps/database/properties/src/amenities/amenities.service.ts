...
export class AmenitiesService {
  constructor(
    ...
    @InjectRepository(EntityAmenities)
    private entityAmenityRepository: Repository<EntityAmenities>,
  ) {}
  ...
  ): Promise<PageDto<AmenitiesDto>> {
    const options = plainToInstance(PageOptionsDto, pageOptionsDto);
    const queryBuilder =
      this.amenitiesRepository.createQueryBuilder('amenities');
  ...
  async findOne(id: string): Promise<AmenitiesDto> {
    ...
    return plainToInstance(AmenitiesDto, amenities, {
  ...
  async update(
    ...
    const updateAmenities = this.amenitiesRepository.merge(
      amenities,
      updateAmenitiesDto,
    );
    await this.amenitiesRepository.save(updateAmenities);

    return plainToInstance(AmenitiesDto, updateAmenities, {
      excludeExtraneousValues: false,
    });
  }
